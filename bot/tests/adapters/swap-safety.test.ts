...
describe("Swap Safety (M0)", () => {
  ...
  it("throws when LIVE_TRADING=true and dryRun=false (real path not implemented)", async () => {
    process.env.LIVE_TRADING = "true";
    await expect(executeSwap(baseIntent)).rejects.toThrow(/not implemented|paper-trade/);
  });
