...
import { getMicroLiveControlSnapshot, armMicroLive } from "../../runtime/live-control.js";
...
export function controlRoutes(deps: ControlRouteDeps = {}): FastifyPluginAsync {
  ...
  return async (fastify) => {
    ...
    }>("/control/arm-live", async (request, reply) => {
      const body = (request.body ?? {}) as { reason?: string };
      const result = armMicroLive(body.reason ?? "control_api");
      return reply.status(result.success ? 200 : 409).send({
