/**
 * Qwen advisory provider — optional; OpenAI-compatible API.
 */
...
export class QwenAdvisoryProvider implements AdvisoryLLMProvider {
  ...
  async generate(pack: AdvisoryEvidencePack): Promise<AdvisoryLLMResponse> {
    ...
    const completion = await client.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `${prompt}\n\nEcho provider as "qwen" and model as "${this.options.model}" in your JSON.`,
        },
      ],
    ...
    try {
      ...
      throw new Error("ADVISORY_QWEN_JSON_PARSE");
    }
    const base = parseAdvisoryLLMResponse(parsed);
    if (!base) {
      throw new Error("ADVISORY_QWEN_SCHEMA");
    }
    return {
      ...base,
      provider: "qwen",
